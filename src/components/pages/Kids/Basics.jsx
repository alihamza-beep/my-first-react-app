...
function KidsSubPage({ title = "Basics" }) {
  ...
  // Professional images pool for Kids Basics
  const imagesPool = [
  ...
  // Generating products dynamically for the Basics collection
  const initialProducts = useMemo(() => Array.from({ length: 12 }).map((_, i) => ({
    id: `kids-basics-${i}`,
    name: `Kids Basics - Essential Art ${i + 1}`,
    price: 2850 + (i * 100), // Slightly lower pricing for basic wear
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Luxury Header - Set to Basics */}
      <div className="text-center mb-4 mt-4">
        ...
        <p className="text-muted small text-uppercase tracking-widest" style={{ fontSize: '10px' }}>Kids Collection / Eastern / {title}</p>
      </div>
