...
function KidsSubPage({ title = "Silky Smiles" }) {
  ...
  // Standard images pool for Kids Collection
  const imagesPool = [
  ...
  const initialProducts = useMemo(() => Array.from({ length: 12 }).map((_, i) => ({
    id: `silky-smiles-${i}`,
    name: `${title} Edition Article ${i + 1}`,
    price: 3800 + (i * 150), 
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Header - Now in Professional Black */}
      <div className="text-center mb-4 mt-4">
        ...
        <p className="text-muted small text-uppercase tracking-widest" style={{ fontSize: '10px' }}>Kids Collection / {title}</p>
      </div>
