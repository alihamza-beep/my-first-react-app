...
function KidsSubPage({ title = "Tops" }) {
  ...
  // Professional images pool for Kids Tops/Western Wear
  const imagesPool = [
  ...
  // Generating products dynamically for the Tops collection
  const initialProducts = useMemo(() => Array.from({ length: 12 }).map((_, i) => ({
    id: `kids-tops-${i}`,
    name: `Kids Signature Top - Art ${i + 1}`,
    price: 2250 + (i * 150), 
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Luxury Header - Set to Tops */}
