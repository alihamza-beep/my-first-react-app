...
function KidsSubPage({ title = "Formals" }) {
  ...
  // Luxury/Formal images pool for Kids
  const imagesPool = [
    "/imgs/Riwayat.webp",
  ...
  // Generating products dynamically for the Formals collection
  const initialProducts = useMemo(() => Array.from({ length: 12 }).map((_, i) => ({
    id: `kids-formals-${i}`,
    name: `Kids Formal - Festive Art ${i + 1}`,
    price: 4500 + (i * 300), // Higher pricing for formal/festive wear
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Luxury Header - Set to Formals */}
