...
function Zarish() {
  ...
  const initialProducts = useMemo(() => Array.from({ length: 12 }).map((_, i) => ({
    id: `zarish-${i}`,
    name: `Zarish Handcrafted Edition ${i + 1}`,
    price: 18500 + (i * 100),
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      ...
      <div className="text-center mb-4 mt-4">
        <h2 className="fw-bold tracking-[0.4em] text-uppercase font-serif" style={{ fontSize: '28px' }}>ZARISH</h2>
        <div className="mx-auto bg-dark mt-2 mb-2" style={{ width: '40px', height: '2px' }}></div>
        <p className="text-muted small text-uppercase tracking-widest" style={{ fontSize: '10px' }}>Luxury Pret / Zarish</p>
      </div>
        ...
        {sortedProducts.map((p) => (
          <div key={p.id} className="col-6 col-md-3">
            <div className="product-card position-relative border-0 group overflow-hidden bg-white shadow-sm pb-3 h-100 d-flex flex-column">
              ...
              {/* --- WISHLIST HEART ICON (Original Circle Style) --- */}
              <button 
