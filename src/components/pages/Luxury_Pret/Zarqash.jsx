...
function LuxuryPretPage({ title = "Zarqash" }) {
  ...
  // Luxury images pool from your local assets
  const imagesPool = [
    "/imgs/Rohani.webp",
  ...
  // Generating products specifically for Zarqash Collection
  const initialProducts = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
    id: `zarqash-${i}`,
    name: `Zarqash Edition Article ${i + 1}`,
    price: 19500 + (i * 300), 
