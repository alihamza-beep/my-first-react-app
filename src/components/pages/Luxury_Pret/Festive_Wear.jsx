...
function LuxuryPretPage({ title = "Festive Wear" }) {
  ...
  // Luxury images pool specifically for Festive Wear
  const imagesPool = [
    "/imgs/Riwayat.webp",
    "/imgs/Rohani.webp",
  ...
  // Generating products based on the Festive Wear theme
  const initialProducts = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
    id: `festive-wear-${i}`,
    name: `Festive Edition Article ${i + 1}`,
    price: 16500 + (i * 400), 
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Header for Festive Wear */}
