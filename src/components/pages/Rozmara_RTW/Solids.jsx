...
function RozmaraSubPage({ title = "Solids" }) {
  ...
  // Professional images pool - Solids specific
  const imagesPool = [
  ...
  // Generating products based on the "Solids" title
  const initialProducts = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
    id: `solids-${i}`,
    name: `Solids - Daily Wear ${i + 1}`,
    price: 4850 + (i * 100), 
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Header Section for Solids */}
