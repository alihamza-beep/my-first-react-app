...
function Khaddar() {
  ...
  // Khaddar/Winter specific images from your assets
  const imagesPool = [
    "/imgs/Rohani.webp",
    "/imgs/Riwayat.webp",
  ...
  // Generating 8 items for the Khaddar collection
  const initialProducts = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
    id: `khaddar-${i}`,
    name: `Khaddar - Winter Wear Article ${i + 1}`,
    price: 5500 + (i * 150), 
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Header Section - Khaddar Title */}
      <div className="text-center mb-4 mt-4">
        <h2 className="fw-bold tracking-[0.4em] text-uppercase font-serif" style={{ fontSize: '28px' }}>KHADDAR</h2>
        <div className="mx-auto bg-dark mt-2 mb-2" style={{ width: '40px', height: '2px' }}></div>
        <p className="text-muted small text-uppercase tracking-widest" style={{ fontSize: '10px' }}>Rozmara RTW / Khaddar Collection</p>
      </div>
