...
function Embroidered() {
  ...
  // Embroidered specific images from your assets
  const imagesPool = [
    "/imgs/Riwayat.webp",
    "/imgs/Rohani.webp",
  ...
  // Generating 8 items for the Embroidered collection
  const initialProducts = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
    id: `embroidered-${i}`,
    name: `Embroidered Luxe Article ${i + 1}`,
    price: 6500 + (i * 200), // Slightly higher price for embroidered work
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      ...
      <div className="text-center mb-4 mt-4">
        <h2 className="fw-bold tracking-[0.4em] text-uppercase font-serif" style={{ fontSize: '28px' }}>Embroidered</h2>
        <div className="mx-auto bg-dark mt-2 mb-2" style={{ width: '40px', height: '2px' }}></div>
        <p className="text-muted small text-uppercase tracking-widest" style={{ fontSize: '10px' }}>Rozmara RTW / Embroidered</p>
      </div>
