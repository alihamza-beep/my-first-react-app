...
function RozmaraSubPage({ title = "Prints" }) {
  ...
  // Professional images pool - Prints specific
  const imagesPool = [
    "/imgs/Prints_1.webp",
    "/imgs/Rohani.webp",
    "/imgs/Riwayat.webp",
  ...
  // Generating products based on the "Prints" title
  const initialProducts = useMemo(() => Array.from({ length: 8 }).map((_, i) => ({
    id: `prints-${i}`,
    name: `Prints - Daily Wear Article ${i + 1}`,
    price: 4250 + (i * 100), // Adjusted price for printed lawn/khaddar
  ...
  return (
    <div className="container-fluid px-4 mt-2 pt-2 bg-white">
      {/* Dynamic Header Section for Prints */}
      <div className="text-center mb-4 mt-4">
        ...
        <p className="text-muted small text-uppercase tracking-widest" style={{ fontSize: '10px' }}>Rozmara RTW / {title}</p>
      </div>
      ...
      <div className="d-flex justify-content-between align-items-center py-3 mb-4 border-top border-bottom">
        ...
        <div className="d-flex align-items-center">
          ...
          >
            <option value="featured">New in Prints</option>
